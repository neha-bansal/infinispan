...
public class CacheLevelLoggingListener {
  ...
  /*
   * In this method we are using annotations to be notified when a cache entry is visited, modified,
  ...
  public void entryPassivationActivation(Event<?, ?> event) {
    switch (event.getType()) {
      case CACHE_ENTRY_PASSIVATED:
        log.info("[Event CACHE_ENTRY_PASSIVATED] Cache entry Passivated : " + ((CacheEntryPassivatedEvent<?, ?>)event).getKey());
        break;
      case CACHE_ENTRY_ACTIVATED:
        log.info("[Event CACHE_ENTRY_ACTIVATED] Cache entry Activated : " + ((CacheEntryPassivatedEvent<?, ?>)event).getKey());
        break;
    }
  }
